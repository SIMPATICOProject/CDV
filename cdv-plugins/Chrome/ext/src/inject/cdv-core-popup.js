...
var cdvCORE = (function () {
	...
	function Singleton() {
				...
				success: function (resp) {
					...
					if (consent) {
						$.ajax({
							...
							beforeSend: function (xhr) {
								xhr.setRequestHeader('Authorization', 'Bearer ' + JSON.parse(localStorage.aacTokenData).access_token);
							},
							success: function (datax) {
					            ...
					            var datamapping= cr_commonpart.rs_description.resource_set.dataset[cr_commonpart.rs_description.resource_set.dataset.length-1].dataMapping;
		...
		this.cdv_getConsentForm = function ( callback) {
			...
			$.ajax({
				type: "GET",
				url: url+accountData+"/"+serviceID+"/"+serviceID+"/"+datasetId,
				contentType: "application/json; charset=utf-8",
				beforeSend: function (xhr) {
					xhr.setRequestHeader('Authorization', 'Bearer ' + JSON.parse(localStorage.aacTokenData).access_token);
				},
