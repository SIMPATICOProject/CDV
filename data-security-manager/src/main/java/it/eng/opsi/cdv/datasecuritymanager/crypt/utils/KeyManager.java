...
public abstract class KeyManager {
	...
	static{
		...
		Logger.getLogger(CryptoUtils.class.getName()).info("User key length setted up to: "+userkey_byte_size+" bytes");
	}

	private static SecretKey generateUserKey() 
			throws Exception {

		SecretKey sk = CryptoUtils.generateRandomKey(userkey_byte_size, GregorianCalendar.getInstance().getTimeInMillis(), Config.get("crypt.key.keyspec"));
		return sk;
	}
	...
	private static SecretKey getUserKey(String userid) 
			throws Exception {
	...
	private static SecretKey getAppKey(String id) 
			throws Exception {
	...
	private static SecretKey generateEncryptionKey(final String id, EncryptionLevel level) 
			throws Exception {
		    ...
		    try{
		    	...
		    	keys.put(id, encryption_key);
		 		...
		 		t.schedule(new TimerTask() {
		 			...
		 			@Override
		 			public void run() {
		 				Logger.getLogger(CryptoUtils.class.getName()).info("Removing encryption key for " + level.toString() + id);
		 				keys.remove(id);
