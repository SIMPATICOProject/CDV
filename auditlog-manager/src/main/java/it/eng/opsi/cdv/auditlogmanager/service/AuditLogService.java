...
public class AuditLogService implements IAuditLogService {
	    			...
	    			if (status == 200) {
	    				...
	    				throw new Exception(
	    						"There was an error while calling the Data Security Manager with Status: " + status
	    								+ " and Response: " + res + " url "+ PropertyManager.getProperty("DATASECURITYMANAGER_HOST") + "/api/internal/encryptValue");
	    			}
			}
	        catch (Exception e) {
				e.printStackTrace();
	...
	public Response getLogs() {
		...
		try {
				entries = dao.getLogs();
				return Response.status(Response.Status.OK)
				.entity(DAOUtils.obj2Json(entries, new TypeToken<ArrayList<AuditLog>>() {		}.getType())).build();
	...
	public Response getLogsByUsernameAndType(@PathParam("username") String username, @PathParam("type") String type) {
		...
		try {
				...
				Client client = ClientBuilder.newClient();
				WebTarget webTarget = client
						.target(PropertyManager.getProperty("DATASECURITYMANAGER_HOST") + "/api/internal/decryptAuditLogList");
				...
				if (status == 200) {
					...
					/*
					return Response.status(Response.Status.OK)
							.entity(DAOUtils.json2Obj(res, new TypeToken<ArrayList<AuditLog>>() {		}.getType())).build();
